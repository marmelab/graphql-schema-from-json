(function (global, factory) {
	...
	(factory((global['json-to-graphql'] = {})));
...
var Source = exports.Source = function Source(body, name, locationOffset) {
  ...
  !(this.locationOffset.line > 0) ? (_invariant2.default)(0, 'line in locationOffset is 1-indexed and must be positive') : void 0;
  !(this.locationOffset.column > 0) ? (_invariant2.default)(0, 'column in locationOffset is 1-indexed and must be positive') : void 0;
};
});
...
message, nodes, source, positions, path, originalError) {
  ...
  if (_source2 && _positions) {
    _locations = _positions.map(function (pos) {
      return (_location.getLocation)(_source2, pos);
    });
  }
...
function syntaxError(source, position, description) {
  var location$$2 = (_location.getLocation)(source, position);
...
function formatError(error) {
  !error ? (_invariant2.default)(0, 'Received null or undefined error.') : void 0;
  return {
...
function readToken(lexer, prev) {
  ...
  if (code < 0x0020 && code !== 0x0009 && code !== 0x000A && code !== 0x000D) {
    throw (_error.syntaxError)(source, position, 'Cannot contain the invalid character ' + printCharCode(code) + '.');
  }
  ...
  throw (_error.syntaxError)(source, position, unexpectedCharacterMessage(code));
}
...
function readNumber(source, start, firstCode, line, col, prev) {
  ...
  if (code === 48) {
    ...
    code = charCodeAt.call(body, ++position);
    if (code >= 48 && code <= 57) {
      throw (_error.syntaxError)(source, position, 'Invalid number, unexpected digit after 0: ' + printCharCode(code) + '.');
    }
  } else {
...
function readDigits(source, start, firstCode) {
  ...
  throw (_error.syntaxError)(source, position, 'Invalid number, expected digit but got: ' + printCharCode(code) + '.');
}
...
function readString(source, start, line, col, prev) {
  ...
  code !== 34) {
    ...
    if (code < 0x0020 && code !== 0x0009) {
      throw (_error.syntaxError)(source, position, 'Invalid character within String: ' + printCharCode(code) + '.');
    }
    ...
    if (code === 92) {
      ...
      switch (code) {
        ...
        case 117:
          ...
          if (charCode < 0) {
            throw (_error.syntaxError)(source, position, 'Invalid character escape sequence: ' + ('\\u' + body.slice(position + 1, position + 5) + '.'));
          }
          value += String.fromCharCode(charCode);
          position += 4;
          break;
        default:
          throw (_error.syntaxError)(source, position, 'Invalid character escape sequence: \\' + String.fromCharCode(code) + '.');
      }
      ++position;
      chunkStart = position;
    }
  ...
  if (code !== 34) {
    // quote (")
    throw (_error.syntaxError)(source, position, 'Unterminated string.');
  }
...
function parse(source$$1, options) {
  ...
  var lexer$$1 = (_lexer.createLexer)(sourceObj, options || {});
...
function expect(lexer$$1, kind) {
  ...
  throw (_error.syntaxError)(lexer$$1.source, token.start, 'Expected ' + kind + ', found ' + (_lexer.getTokenDesc)(token));
}
...
function expectKeyword(lexer$$1, value) {
  ...
  throw (_error.syntaxError)(lexer$$1.source, token.start, 'Expected "' + value + '", found ' + (_lexer.getTokenDesc)(token));
}
...
function unexpected(lexer$$1, atToken) {
  ...
  return (_error.syntaxError)(lexer$$1.source, token.start, 'Unexpected ' + (_lexer.getTokenDesc)(token));
}
...
function assertType(type) {
  !isType(type) ? (_invariant2.default)(0, 'Expected ' + String(type) + ' to be a GraphQL type.') : void 0;
  return type;
}
...
function assertInputType(type) {
  !isInputType(type) ? (_invariant2.default)(0, 'Expected ' + String(type) + ' to be a GraphQL input type.') : void 0;
  return type;
}
...
function assertOutputType(type) {
  !isOutputType(type) ? (_invariant2.default)(0, 'Expected ' + String(type) + ' to be a GraphQL output type.') : void 0;
  return type;
}
...
function assertLeafType(type) {
  !isLeafType(type) ? (_invariant2.default)(0, 'Expected ' + String(type) + ' to be a GraphQL leaf type.') : void 0;
  return type;
}
...
function assertCompositeType(type) {
  !isCompositeType(type) ? (_invariant2.default)(0, 'Expected ' + String(type) + ' to be a GraphQL composite type.') : void 0;
  return type;
}
...
function assertAbstractType(type) {
  !isAbstractType(type) ? (_invariant2.default)(0, 'Expected ' + String(type) + ' to be a GraphQL abstract type.') : void 0;
  return type;
}
...
function assertNamedType(type) {
  !isNamedType(type) ? (_invariant2.default)(0, 'Expected ' + String(type) + ' to be a GraphQL named type.') : void 0;
  return type;
}
...
var GraphQLScalarType = exports.GraphQLScalarType = function () {
  function GraphQLScalarType(config) {
    ...
    (_assertValidName.assertValidName)(config.name);
    this.name = config.name;
    this.description = config.description;
    this.astNode = config.astNode;
    !(typeof config.serialize === 'function') ? (_invariant2.default)(0, this.name + ' must provide "serialize" function. If this custom Scalar ' + 'is also used as an input type, ensure "parseValue" and "parseLiteral" ' + 'functions are also provided.') : void 0;
    if (config.parseValue || config.parseLiteral) {
      !(typeof config.parseValue === 'function' && typeof config.parseLiteral === 'function') ? (_invariant2.default)(0, this.name + ' must provide both "parseValue" and "parseLiteral" ' + 'functions.') : void 0;
    }
  ...
  GraphQLScalarType.prototype.isValidValue = function isValidValue(value) {
    return !(_isNullish2.default)(this.parseValue(value));
  };
  ...
  GraphQLScalarType.prototype.parseValue = function parseValue(value) {
    ...
    return parser && !(_isNullish2.default)(value) ? parser(value) : undefined;
  };
  ...
  GraphQLScalarType.prototype.isValidLiteral = function isValidLiteral(valueNode) {
    return !(_isNullish2.default)(this.parseLiteral(valueNode));
  };
...
var GraphQLObjectType = exports.GraphQLObjectType = function () {
  function GraphQLObjectType(config) {
    ...
    (_assertValidName.assertValidName)(config.name, config.isIntrospection);
    this.name = config.name;
    this.description = config.description;
    this.astNode = config.astNode;
    this.extensionASTNodes = config.extensionASTNodes || [];
    if (config.isTypeOf) {
      !(typeof config.isTypeOf === 'function') ? (_invariant2.default)(0, this.name + ' must provide "isTypeOf" as a function.') : void 0;
    }
...
function defineInterfaces(type, interfacesThunk) {
  ...
  !Array.isArray(interfaces) ? (_invariant2.default)(0, type.name + ' interfaces must be an Array or a function which returns ' + 'an Array.') : void 0;

  var implementedTypeNames = Object.create(null);
  interfaces.forEach(function (iface) {
    !(iface instanceof GraphQLInterfaceType) ? (_invariant2.default)(0, type.name + ' may only implement Interface types, it cannot ' + ('implement: ' + String(iface) + '.')) : void 0;
    !!implementedTypeNames[iface.name] ? (_invariant2.default)(0, type.name + ' may declare it implements ' + iface.name + ' only once.') : void 0;
    implementedTypeNames[iface.name] = true;
    if (typeof iface.resolveType !== 'function') {
      !(typeof type.isTypeOf === 'function') ? (_invariant2.default)(0, 'Interface Type ' + iface.name + ' does not provide a "resolveType" ' + ('function and implementing Type ' + type.name + ' does not provide a ') + '"isTypeOf" function. There is no way to resolve this implementing ' + 'type during execution.') : void 0;
    }
  });
  return interfaces;
}
...
function defineFieldMap(type, fieldsThunk) {
  ...
  !isPlainObj(fieldMap) ? (_invariant2.default)(0, type.name + ' fields must be an object with field names as keys or a ' + 'function which returns such an object.') : void 0;

  var fieldNames = Object.keys(fieldMap);
  !(fieldNames.length > 0) ? (_invariant2.default)(0, type.name + ' fields must be an object with field names as keys or a ' + 'function which returns such an object.') : void 0;

  var resultFieldMap = Object.create(null);
  fieldNames.forEach(function (fieldName) {
    (_assertValidName.assertValidName)(fieldName);
    var fieldConfig = fieldMap[fieldName];
    !isPlainObj(fieldConfig) ? (_invariant2.default)(0, type.name + '.' + fieldName + ' field config must be an object') : void 0;
    !!fieldConfig.hasOwnProperty('isDeprecated') ? (_invariant2.default)(0, type.name + '.' + fieldName + ' should provide "deprecationReason" instead ' + 'of "isDeprecated".') : void 0;
    var field = _extends({}, fieldConfig, {
      isDeprecated: Boolean(fieldConfig.deprecationReason),
      name: fieldName
    });
    !isOutputType(field.type) ? (_invariant2.default)(0, type.name + '.' + fieldName + ' field type must be Output Type but ' + ('got: ' + String(field.type) + '.')) : void 0;
    !isValidResolver(field.resolve) ? (_invariant2.default)(0, type.name + '.' + fieldName + ' field resolver must be a function if ' + ('provided, but got: ' + String(field.resolve) + '.')) : void 0;
    var argsConfig = fieldConfig.args;
    if (!argsConfig) {
      field.args = [];
    } else {
      !isPlainObj(argsConfig) ? (_invariant2.default)(0, type.name + '.' + fieldName + ' args must be an object with argument ' + 'names as keys.') : void 0;
      field.args = Object.keys(argsConfig).map(function (argName) {
        (_assertValidName.assertValidName)(argName);
        var arg = argsConfig[argName];
        !isInputType(arg.type) ? (_invariant2.default)(0, type.name + '.' + fieldName + '(' + argName + ':) argument type must be ' + ('Input Type but got: ' + String(arg.type) + '.')) : void 0;
        return {
          name: argName,
...
var GraphQLInterfaceType = exports.GraphQLInterfaceType = function () {
  function GraphQLInterfaceType(config) {
    ...
    if (config.resolveType) {
      !(typeof config.resolveType === 'function') ? (_invariant2.default)(0, this.name + ' must provide "resolveType" as a function.') : void 0;
    }
...
function defineTypes(unionType, typesThunk) {
  ...
  !(Array.isArray(types) && types.length > 0) ? (_invariant2.default)(0, 'Must provide Array of types or a function which returns ' + ('such an array for Union ' + unionType.name + '.')) : void 0;
  var includedTypeNames = Object.create(null);
  types.forEach(function (objType) {
    !(objType instanceof GraphQLObjectType) ? (_invariant2.default)(0, unionType.name + ' may only contain Object types, it cannot contain: ' + (String(objType) + '.')) : void 0;
    !!includedTypeNames[objType.name] ? (_invariant2.default)(0, unionType.name + ' can include ' + objType.name + ' type only once.') : void 0;
    includedTypeNames[objType.name] = true;
    if (typeof unionType.resolveType !== 'function') {
      !(typeof objType.isTypeOf === 'function') ? (_invariant2.default)(0, 'Union type "' + unionType.name + '" does not provide a "resolveType" ' + ('function and possible type "' + objType.name + '" does not provide an ') + '"isTypeOf" function. There is no way to resolve this possible type ' + 'during execution.') : void 0;
    }
  });
...
function defineEnumValues(type, valueMap /* <T> */
  ...
  !isPlainObj(valueMap) ? (_invariant2.default)(0, type.name + ' values must be an object with value names as keys.') : void 0;
  var valueNames = Object.keys(valueMap);
  !(valueNames.length > 0) ? (_invariant2.default)(0, type.name + ' values must be an object with value names as keys.') : void 0;
  return valueNames.map(function (valueName) {
    (_assertValidName.assertValidName)(valueName);
    !(['true', 'false', 'null'].indexOf(valueName) === -1) ? (_invariant2.default)(0, 'Name "' + valueName + '" can not be used as an Enum value.') : void 0;

    var value = valueMap[valueName];
    !isPlainObj(value) ? (_invariant2.default)(0, type.name + '.' + valueName + ' must refer to an object with a "value" key ' + ('representing an internal value but got: ' + String(value) + '.')) : void 0;
    !!value.hasOwnProperty('isDeprecated') ? (_invariant2.default)(0, type.name + '.' + valueName + ' should provide "deprecationReason" instead ' + 'of "isDeprecated".') : void 0;
    return {
      name: valueName,
...
var GraphQLInputObjectType = exports.GraphQLInputObjectType = function () {
  ...
  GraphQLInputObjectType.prototype._defineFieldMap = function _defineFieldMap() {
    ...
    !isPlainObj(fieldMap) ? (_invariant2.default)(0, this.name + ' fields must be an object with field names as keys or a ' + 'function which returns such an object.') : void 0;
    var fieldNames = Object.keys(fieldMap);
    !(fieldNames.length > 0) ? (_invariant2.default)(0, this.name + ' fields must be an object with field names as keys or a ' + 'function which returns such an object.') : void 0;
    var resultFieldMap = Object.create(null);
    fieldNames.forEach(function (fieldName) {
      (_assertValidName.assertValidName)(fieldName);
      ...
      !isInputType(field.type) ? (_invariant2.default)(0, _this.name + '.' + fieldName + ' field type must be Input Type but ' + ('got: ' + String(field.type) + '.')) : void 0;
      !(field.resolve == null) ? (_invariant2.default)(0, _this.name + '.' + fieldName + ' field type has a resolve property, but ' + 'Input Types cannot define resolvers.') : void 0;
      resultFieldMap[fieldName] = field;
    });
    return resultFieldMap;
  };
...
var GraphQLList = exports.GraphQLList = function () {
  function GraphQLList(type) {
    ...
    !isType(type) ? (_invariant2.default)(0, 'Can only create List of a GraphQLType but got: ' + String(type) + '.') : void 0;
    this.ofType = type;
  }
...
var GraphQLNonNull = exports.GraphQLNonNull = function () {
  function GraphQLNonNull(type) {
    ...
    !(isType(type) && !(type instanceof GraphQLNonNull)) ? (_invariant2.default)(0, 'Can only create NonNull of a Nullable GraphQLType but got: ' + (String(type) + '.')) : void 0;
    this.ofType = type;
  }
...
var GraphQLDirective = exports.GraphQLDirective = function GraphQLDirective(config) {
  ...
  !config.name ? (_invariant2.default)(0, 'Directive must be named.') : void 0;
  (_assertValidName.assertValidName)(config.name);
  !Array.isArray(config.locations) ? (_invariant2.default)(0, 'Must provide locations for directive.') : void 0;
  this.name = config.name;
  this.description = config.description;
  ...
  if (!args) {
    ...
    !!Array.isArray(args) ? (_invariant2.default)(0, '@' + config.name + ' args must be an object with argument names as keys.') : void 0;
    this.args = Object.keys(args).map(function (argName) {
      (_assertValidName.assertValidName)(argName);
      var arg = args[argName];
      !(_definition.isInputType)(arg.type) ? (_invariant2.default)(0, '@' + config.name + '(' + argName + ':) argument type must be ' + ('Input Type but got: ' + String(arg.type) + '.')) : void 0;
      return {
        name: argName,
        description: arg.description === undefined ? null : arg.description,
        type: arg.type,
...
function astFromValue(value, type) {
  ...
  if (type instanceof definition.GraphQLList) {
    ...
    if ((_iterall.isCollection)(_value)) {
      var valuesNodes = [];
      (_iterall.forEach)(_value, function (item) {
  ...
  !(type instanceof definition.GraphQLScalarType || type instanceof definition.GraphQLEnumType) ? (_invariant2.default)(0, 'Must provide Input Type, cannot use: ' + String(type)) : void 0;

  // Since value is an internally represented value, it must be serialized
  // to an externally represented value before converting into an AST.
  var serialized = type.serialize(_value);
  if ((_isNullish2.default)(serialized)) {
    return null;
  }
 ...
 */
function print(ast) {
  return (_visitor.visit)(ast, { leave: printDocASTReducer });
} /**
   * Copyright (c) 2015-present, Facebook, Inc.
   *
   * This source code is licensed under the MIT license found in the
    ...
    return {
      ...
      possibleTypes: {
        ...
        resolve: function resolve(type, args, context, _ref2) {
          ...
          if ((_definition.isAbstractType)(type)) {
    ...
    return {
      ...
      defaultValue: {
        ...
        resolve: function resolve(inputVal) {
          return (_isInvalid2.default)(inputVal.defaultValue) ? null : (_printer.print)((_astFromValue.astFromValue)(inputVal.defaultValue, inputVal.type));
        }
      }
    };
  }
...
function isTypeSubTypeOf(schema, maybeSubType, superType) {
  ...
  if ((_definition.isAbstractType)(superType) && maybeSubType instanceof definition.GraphQLObjectType && schema.isPossibleType(superType, maybeSubType)) {
    return true;
  }
...
function doTypesOverlap(schema, typeA, typeB) {
  ...
  if ((_definition.isAbstractType)(typeA)) {
    if ((_definition.isAbstractType)(_typeB)) {
...
var GraphQLSchema = exports.GraphQLSchema = function () {
  function GraphQLSchema(config) {
    ...
    !((typeof config === 'undefined' ? 'undefined' : _typeof(config)) === 'object') ? (_invariant2.default)(0, 'Must provide configuration object.') : void 0;

    !(config.query instanceof definition.GraphQLObjectType) ? (_invariant2.default)(0, 'Schema query must be Object Type but got: ' + String(config.query) + '.') : void 0;
    this._queryType = config.query;

    !(!config.mutation || config.mutation instanceof definition.GraphQLObjectType) ? (_invariant2.default)(0, 'Schema mutation must be Object Type if provided but got: ' + String(config.mutation) + '.') : void 0;
    this._mutationType = config.mutation;

    !(!config.subscription || config.subscription instanceof definition.GraphQLObjectType) ? (_invariant2.default)(0, 'Schema subscription must be Object Type if provided but got: ' + String(config.subscription) + '.') : void 0;
    this._subscriptionType = config.subscription;

    !(!config.types || Array.isArray(config.types)) ? (_invariant2.default)(0, 'Schema types must be Array if provided but got: ' + String(config.types) + '.') : void 0;
    ...
    })) ? (_invariant2.default)(0, 'Schema directives must be Array<GraphQLDirective> if provided but got: ' + String(config.directives) + '.') : void 0;
  ...
  GraphQLSchema.prototype.getPossibleTypes = function getPossibleTypes(abstractType) {
    ...
    !(abstractType instanceof definition.GraphQLInterfaceType) ? (_invariant2.default)(0) : void 0;
  ...
  GraphQLSchema.prototype.isPossibleType = function isPossibleType(abstractType, possibleType) {
    ...
    if (!possibleTypeMap[abstractType.name]) {
      ...
      !Array.isArray(possibleTypes) ? (_invariant2.default)(0, 'Could not find possible implementing types for ' + abstractType.name + ' ' + 'in schema. Check that schema.types is defined and is an array of ' + 'all possible types in the schema.') : void 0;
  ...
  GraphQLSchema.prototype.getDirective = function getDirective(name) {
    return (_find2.default)(this.getDirectives(), function (directive) {
...
function typeMapReducer(map, type) {
  ...
  if (map[type.name]) {
    !(map[type.name] === type) ? (_invariant2.default)(0, 'Schema must contain unique named types but contains multiple ' + ('types named "' + type.name + '".')) : void 0;
    return map;
  }
  map[type.name] = type;
...
function assertObjectImplementsInterface(schema, object, iface) {
  ...
  Object.keys(ifaceFieldMap).forEach(function (fieldName) {
    ...
    !objectField ? (_invariant2.default)(0, '"' + iface.name + '" expects field "' + fieldName + '" but "' + object.name + '" ' + 'does not provide it.') : void 0;

    // Assert interface field type is satisfied by object field type, by being
    // a valid subtype. (covariant)
    !(_typeComparators.isTypeSubTypeOf)(schema, objectField.type, ifaceField.type) ? (_invariant2.default)(0, iface.name + '.' + fieldName + ' expects type "' + String(ifaceField.type) + '" ' + 'but ' + (object.name + '.' + fieldName + ' provides type "' + String(objectField.type) + '".')) : void 0;

    // Assert each interface field arg is implemented.
    ifaceField.args.forEach(function (ifaceArg) {
      var argName = ifaceArg.name;
      var objectArg = (_find2.default)(objectField.args, function (arg) {
        return arg.name === argName;
      });

      // Assert interface field arg exists on object field.
      !objectArg ? (_invariant2.default)(0, iface.name + '.' + fieldName + ' expects argument "' + argName + '" but ' + (object.name + '.' + fieldName + ' does not provide it.')) : void 0;

      // Assert interface field arg type matches object field arg type.
      // (invariant)
      !(_typeComparators.isEqualType)(ifaceArg.type, objectArg.type) ? (_invariant2.default)(0, iface.name + '.' + fieldName + '(' + argName + ':) expects type ' + ('"' + String(ifaceArg.type) + '" but ') + (object.name + '.' + fieldName + '(' + argName + ':) provides type ') + ('"' + String(objectArg.type) + '".')) : void 0;
    });

    // Assert additional arguments must not be required.
    objectField.args.forEach(function (objectArg) {
      var argName = objectArg.name;
      var ifaceArg = (_find2.default)(ifaceField.args, function (arg) {
        return arg.name === argName;
      });
      if (!ifaceArg) {
        !!(objectArg.type instanceof definition.GraphQLNonNull) ? (_invariant2.default)(0, object.name + '.' + fieldName + '(' + argName + ':) is of required type ' + ('"' + String(objectArg.type) + '" but is not also provided by the ') + ('interface ' + iface.name + '.' + fieldName + '.')) : void 0;
      }
    });
  });
}
...
function typeFromASTImpl(schema, typeNode) {
  ...
  !(typeNode.kind === Kind.NAMED_TYPE) ? (_invariant2.default)(0, 'Must be a named type.') : void 0;
  ...
  TypeInfo.prototype.enter = function enter(node /* ASTNode */) {
    ...
    switch (node.kind) {
      case Kind.SELECTION_SET:
        var namedType = (_definition.getNamedType)(this.getType());
        this._parentTypeStack.push((_definition.isCompositeType)(namedType) ? namedType : undefined);
        break;
      case Kind.FIELD:
      ...
      case Kind.FRAGMENT_DEFINITION:
        ...
        var outputType = typeConditionAST ? (_typeFromAST.typeFromAST)(schema, typeConditionAST) : this.getType();
        this._typeStack.push((_definition.isOutputType)(outputType) ? outputType : undefined);
        break;
      case Kind.VARIABLE_DEFINITION:
        var inputType = (_typeFromAST.typeFromAST)(schema, node.type);
        this._inputTypeStack.push((_definition.isInputType)(inputType) ? inputType : undefined);
        break;
      case Kind.ARGUMENT:
        var argDef = void 0;
        var argType = void 0;
        ...
        if (fieldOrDirective) {
          argDef = (_find2.default)(fieldOrDirective.args, function (arg) {
        ...
        break;
      case Kind.LIST:
        var listType = (_definition.getNullableType)(this.getInputType());
        this._inputTypeStack.push(listType instanceof definition.GraphQLList ? listType.ofType : undefined);
        break;
      case Kind.OBJECT_FIELD:
        var objectType = (_definition.getNamedType)(this.getInputType());
        ...
        break;
      case Kind.ENUM:
        var enumType = (_definition.getNamedType)(this.getInputType());
...
function getFieldDef(schema, parentType, fieldNode) {
  ...
  if (name === introspection.TypeNameMetaFieldDef.name && (_definition.isCompositeType)(parentType)) {
...
function unknownTypeMessage(type, suggestedTypes) {
  ...
  if (suggestedTypes.length) {
    message += ' Did you mean ' + (_quotedOrList2.default)(suggestedTypes) + '?';
  }
  return message;
}
...
function KnownTypeNames(context) {
  return {
    ...
    NamedType: function NamedType(node) {
      ...
      if (!type) {
        context.reportError(new error.GraphQLError(unknownTypeMessage(typeName, (_suggestionList2.default)(typeName, Object.keys(schema.getTypeMap()))), [node]));
      }
    }
  };
}
  ...
  return {
    InlineFragment: function InlineFragment(node) {
      if (node.typeCondition) {
        var type = (_typeFromAST.typeFromAST)(context.getSchema(), node.typeCondition);
        if (type && !(_definition.isCompositeType)(type)) {
          context.reportError(new error.GraphQLError(inlineFragmentOnNonCompositeErrorMessage((_printer.print)(node.typeCondition)), [node.typeCondition]));
        }
      }
    },
    FragmentDefinition: function FragmentDefinition(node) {
      var type = (_typeFromAST.typeFromAST)(context.getSchema(), node.typeCondition);
      if (type && !(_definition.isCompositeType)(type)) {
        context.reportError(new error.GraphQLError(fragmentOnNonCompositeErrorMessage(node.name.value, (_printer.print)(node.typeCondition)), [node.typeCondition]));
      }
    }
  };
}
...
function VariablesAreInputTypes(context) {
  return {
    VariableDefinition: function VariableDefinition(node) {
      var type = (_typeFromAST.typeFromAST)(context.getSchema(), node.type);

      // If the variable type is not an input type, return an error.
      if (type && !(_definition.isInputType)(type)) {
        var variableName = node.variable.name.value;
        context.reportError(new error.GraphQLError(nonInputTypeOnVarMessage(variableName, (_printer.print)(node.type)), [node.type]));
      }
    }
  };
}
...
function ScalarLeafs(context) {
  return {
    Field: function Field(node) {
      ...
      if (type) {
        if ((_definition.isLeafType)((_definition.getNamedType)(type))) {
...
function undefinedFieldMessage(fieldName, type, suggestedTypeNames, suggestedFieldNames) {
  ...
  if (suggestedTypeNames.length !== 0) {
    var suggestions = (_quotedOrList2.default)(suggestedTypeNames);
    message += ' Did you mean to use an inline fragment on ' + suggestions + '?';
  } else if (suggestedFieldNames.length !== 0) {
    message += ' Did you mean ' + (_quotedOrList2.default)(suggestedFieldNames) + '?';
  }
  return message;
}
...
function getSuggestedFieldNames(schema, type, fieldName) {
  if (type instanceof definition.GraphQLObjectType || type instanceof definition.GraphQLInterfaceType) {
    ...
    return (_suggestionList2.default)(fieldName, possibleFieldNames);
  }
...
function PossibleFragmentSpreads(context) {
  return {
    InlineFragment: function InlineFragment(node) {
      ...
      if ((_definition.isCompositeType)(fragType) && (_definition.isCompositeType)(parentType) && !(_typeComparators.doTypesOverlap)(context.getSchema(), fragType, parentType)) {
    ...
    FragmentSpread: function FragmentSpread(node) {
      ...
      var parentType = context.getParentType();
      if (fragType && parentType && !(_typeComparators.doTypesOverlap)(context.getSchema(), fragType, parentType)) {
...
function getFragmentType(context, name) {
  ...
  return frag && (_typeFromAST.typeFromAST)(context.getSchema(), frag.typeCondition);
}
});
...
function KnownDirectives(context) {
  return {
    Directive: function Directive(node, key, parent, path, ancestors) {
      var directiveDef = (_find2.default)(context.getSchema().getDirectives(), function (def) {
...
function unknownArgMessage(argName, fieldName, typeName, suggestedArgs) {
  ...
  if (suggestedArgs.length) {
    message += ' Did you mean ' + (_quotedOrList2.default)(suggestedArgs) + '?';
  }
  return message;
} /**
   * Copyright (c) 2015-present, Facebook, Inc.
  ...
  return {
    Argument: function Argument(node, key, parent, path, ancestors) {
      ...
      if (argumentOf.kind === Kind.FIELD) {
        var fieldDef = context.getFieldDef();
        if (fieldDef) {
          var fieldArgDef = (_find2.default)(fieldDef.args, function (arg) {
            return arg.name === node.name.value;
          });
          if (!fieldArgDef) {
            var parentType = context.getParentType();
            !parentType ? (_invariant2.default)(0) : void 0;
            context.reportError(new error.GraphQLError(unknownArgMessage(node.name.value, fieldDef.name, parentType.name, (_suggestionList2.default)(node.name.value, fieldDef.args.map(function (arg) {
              return arg.name;
            }))), [node]));
          }
        }
        ...
        if (directive) {
          var directiveArgDef = (_find2.default)(directive.args, function (arg) {
            return arg.name === node.name.value;
          });
          if (!directiveArgDef) {
            context.reportError(new error.GraphQLError(unknownDirectiveArgMessage(node.name.value, directive.name, (_suggestionList2.default)(node.name.value, directive.args.map(function (arg) {
              return arg.name;
            }))), [node]));
          }
        }
...
function isValidLiteralValue(type, valueNode) {
  ...
  if (type instanceof definition.GraphQLInputObjectType) {
    ...
    var fieldNodeMap = (_keyMap2.default)(fieldNodes, function (fieldNode) {
  ...
  !(type instanceof definition.GraphQLScalarType || type instanceof definition.GraphQLEnumType) ? (_invariant2.default)(0, 'Must be input type') : void 0;

  // Scalars determine if a literal values is valid.
  if (!type.isValidLiteral(valueNode)) {
    return ['Expected type "' + type.name + '", found ' + (_printer.print)(valueNode) + '.'];
  }
...
function ArgumentsOfCorrectType(context) {
  return {
    Argument: function Argument(node) {
      ...
      if (argDef) {
        var errors = (_isValidLiteralValue.isValidLiteralValue)(argDef.type, node.value);
        if (errors && errors.length > 0) {
          context.reportError(new error.GraphQLError(badValueMessage(node.name.value, argDef.type, (_printer.print)(node.value), errors), [node.value]));
        }
      }
      return false;
    }
  ...
  return {
    Field: {
      ...
      leave: function leave(node) {
        ...
        var argNodeMap = (_keyMap2.default)(argNodes, function (arg) {
  ...
  return {
    VariableDefinition: function VariableDefinition(node) {
      ...
      if (type && defaultValue) {
        var errors = (_isValidLiteralValue.isValidLiteralValue)(type, defaultValue);
        if (errors && errors.length > 0) {
          context.reportError(new error.GraphQLError(badValueForDefaultArgMessage(name, type, (_printer.print)(defaultValue), errors), [defaultValue]));
        }
      }
      return false;
    },
      ...
      leave: function leave(operation) {
        ...
        usages.forEach(function (_ref) {
          ...
          if (varDef && type) {
            ...
            var schema = context.getSchema();
            var varType = (_typeFromAST.typeFromAST)(schema, varDef.type);
            if (varType && !(_typeComparators.isTypeSubTypeOf)(schema, effectiveType(varType, varDef), type)) {
...
function findConflict(context, cachedFieldsAndFragmentNames, comparedFragments, parentFieldsAreMutuallyExclusive, responseName, field1, field2) {
  ...
  if (selectionSet1 && selectionSet2) {
    var conflicts = findConflictsBetweenSubSelectionSets(context, cachedFieldsAndFragmentNames, comparedFragments, areMutuallyExclusive, (_definition.getNamedType)(type1), selectionSet1, (_definition.getNamedType)(type2), selectionSet2);
...
function sameArguments(arguments1, arguments2) {
  ...
  return arguments1.every(function (argument1) {
    var argument2 = (_find2.default)(arguments2, function (argument) {
...
function sameValue(value1, value2) {
  return !value1 && !value2 || (_printer.print)(value1) === (_printer.print)(value2);
}
...
function doTypesConflict(type1, type2) {
  ...
  if ((_definition.isLeafType)(type1) || (_definition.isLeafType)(type2)) {
    return type1 !== type2;
  }
  return false;
}
...
function getReferencedFieldsAndFragmentNames(context, cachedFieldsAndFragmentNames, fragment) {
  ...
  var fragmentType = (_typeFromAST.typeFromAST)(context.getSchema(), fragment.typeCondition);
  ...
  for (var i = 0; i < selectionSet.selections.length; i++) {
    ...
    switch (selection.kind) {
      ...
      case Kind.INLINE_FRAGMENT:
        ...
        var inlineFragmentType = typeCondition ? (_typeFromAST.typeFromAST)(context.getSchema(), typeCondition) : parentType;
...
function validate(schema$$1, ast, rules, typeInfo) {
  !schema$$1 ? (_invariant2.default)(0, 'Must provide schema') : void 0;
  !ast ? (_invariant2.default)(0, 'Must provide document') : void 0;
  !(schema$$1 instanceof schema.GraphQLSchema) ? (_invariant2.default)(0, 'Schema must be an instance of GraphQLSchema. Also ensure that there are ' + 'not multiple versions of GraphQL installed in your node_modules directory.') : void 0;
...
function visitUsingRules(schema$$1, typeInfo, documentAST, rules) {
  ...
  (_visitor.visit)(documentAST, (_visitor.visitWithTypeInfo)(typeInfo, (_visitor.visitInParallel)(visitors)));
...
var ValidationContext = exports.ValidationContext = function () {
  ...
  ValidationContext.prototype.getVariableUsages = function getVariableUsages(node) {
    ...
    if (!usages) {
      ...
      (_visitor.visit)(node, (_visitor.visitWithTypeInfo)(typeInfo, {
...
function valueFromAST(valueNode, type, variables) {
  ...
  if (valueNode.kind === Kind.VARIABLE) {
    ...
    if (!variables || (_isInvalid2.default)(variables[variableName])) {
  ...
  if (type instanceof definition.GraphQLList) {
    ...
    if (valueNode.kind === Kind.LIST) {
      ...
      for (var i = 0; i < itemNodes.length; i++) {
        if (isMissingVariable(itemNodes[i], variables)) {
          ...
          if ((_isInvalid2.default)(itemValue)) {
            return; // Invalid: intentionally return no value.
          }
    ...
    if ((_isInvalid2.default)(coercedValue)) {
      return; // Invalid: intentionally return no value.
    }
    return [coercedValue];
  }
  ...
  if (type instanceof definition.GraphQLInputObjectType) {
    ...
    var fields = type.getFields();
    var fieldNodes = (_keyMap2.default)(valueNode.fields, function (field) {
    ...
    for (var _i = 0; _i < fieldNames.length; _i++) {
      ...
      if (!fieldNode || isMissingVariable(fieldNode.value, variables)) {
        if (!(_isInvalid2.default)(field.defaultValue)) {
      ...
      if ((_isInvalid2.default)(fieldValue)) {
        return; // Invalid: intentionally return no value.
      }
  ...
  if ((_isNullish2.default)(parsed) && !type.isValidLiteral(valueNode)) {
...
function isMissingVariable(valueNode, variables) {
  return valueNode.kind === Kind.VARIABLE && (!variables || (_isInvalid2.default)(variables[valueNode.name.value]));
}
});
...
function isValidJSValue(value, type) {
  ...
  if (type instanceof definition.GraphQLList) {
    var itemType = type.ofType;
    if ((_iterall.isCollection)(value)) {
      var errors = [];
      (_iterall.forEach)(value, function (item, index) {
  ...
  try {
    ...
    if ((_isNullish2.default)(parseResult) && !type.isValidValue(value)) {
...
function getVariableValues(schema, varDefNodes, inputs) {
  ...
  for (var i = 0; i < varDefNodes.length; i++) {
    ...
    var varType = (_typeFromAST.typeFromAST)(schema, varDefNode.type);
    if (!(_definition.isInputType)(varType)) {
      throw new error.GraphQLError('Variable "$' + varName + '" expected value of type ' + ('"' + (_printer.print)(varDefNode.type) + '" which cannot be used as an input type.'), [varDefNode.type]);
    }

    var value = inputs[varName];
    if ((_isInvalid2.default)(value)) {
      var defaultValue = varDefNode.defaultValue;
      if (defaultValue) {
        coercedValues[varName] = (_valueFromAST.valueFromAST)(defaultValue, varType);
      }
      ...
      var errors = (_isValidJSValue.isValidJSValue)(value, varType);
      if (errors.length) {
        var message = errors ? '\n' + errors.join('\n') : '';
      ...
      !!(_isInvalid2.default)(coercedValue) ? (_invariant2.default)(0, 'Should have reported error.') : void 0;
...
function getArgumentValues(def, node, variableValues) {
  ...
  for (var i = 0; i < argDefs.length; i++) {
    ...
    if (!argumentNode) {
      if (!(_isInvalid2.default)(defaultValue)) {
      ...
      if (variableValues && Object.prototype.hasOwnProperty.call(variableValues, variableName) && !(_isInvalid2.default)(variableValues[variableName])) {
        // Note: this does not check that this variable value is correct.
        // This assumes that this query has been validated and the variable
        // usage here is of the correct type.
        coercedValues[name] = variableValues[variableName];
      } else if (!(_isInvalid2.default)(defaultValue)) {
        coercedValues[name] = defaultValue;
      } else if (argType instanceof definition.GraphQLNonNull) {
      ...
      var coercedValue = (_valueFromAST.valueFromAST)(valueNode, argType, variableValues);
      if ((_isInvalid2.default)(coercedValue)) {
        var errors = (_isValidLiteralValue.isValidLiteralValue)(argType, valueNode);
        var message = errors ? '\n' + errors.join('\n') : '';
        throw new error.GraphQLError('Argument "' + name + '" got invalid value ' + (_printer.print)(valueNode) + '.' + message, [argumentNode.value]);
      }
...
function getDirectiveValues(directiveDef, node, variableValues) {
  var directiveNode = node.directives && (_find2.default)(node.directives, function (directive) {
...
function coerceValue(type, value) {
  ...
  if (type instanceof definition.GraphQLList) {
    ...
    if ((_iterall.isCollection)(_value)) {
      var coercedValues = [];
      var valueIter = (_iterall.createIterator)(_value);
      if (!valueIter) {
        return; // Intentionally return no value.
      }
      var step = void 0;
  ...
  if (type instanceof definition.GraphQLInputObjectType) {
    ...
    for (var i = 0; i < fieldNames.length; i++) {
      ...
      var field = fields[fieldName];
      if ((_isInvalid2.default)(_value[fieldName])) {
        if (!(_isInvalid2.default)(field.defaultValue)) {
          coercedObj[fieldName] = field.defaultValue;
        } else if (field.type instanceof definition.GraphQLNonNull) {
          return; // Intentionally return no value.
...
function assertValidExecutionArguments(schema$$2, document, rawVariableValues) {
  !schema$$2 ? (_invariant2.default)(0, 'Must provide schema') : void 0;
  !document ? (_invariant2.default)(0, 'Must provide document') : void 0;
  !(schema$$2 instanceof schema.GraphQLSchema) ? (_invariant2.default)(0, 'Schema must be an instance of GraphQLSchema. Also ensure that there are ' + 'not multiple versions of GraphQL installed in your node_modules directory.') : void 0;

  // Variables, if provided, must be an object.
  !(!rawVariableValues || (typeof rawVariableValues === 'undefined' ? 'undefined' : _typeof(rawVariableValues)) === 'object') ? (_invariant2.default)(0, 'Variables must be provided as an Object where each property is a ' + 'variable value. Perhaps look to see if an unparsed JSON string ' + 'was provided.') : void 0;
}
...
function buildExecutionContext(schema$$2, document, rootValue, contextValue, rawVariableValues, operationName, fieldResolver) {
  ...
  var variableValues = (_values.getVariableValues)(schema$$2, operation.variableDefinitions || [], rawVariableValues || {});
...
function shouldIncludeNode(exeContext, node) {
  var skip = (_values.getDirectiveValues)(directives.GraphQLSkipDirective, node, exeContext.variableValues);
  if (skip && skip.if === true) {
    return false;
  }

  var include = (_values.getDirectiveValues)(directives.GraphQLIncludeDirective, node, exeContext.variableValues);
...
function doesFragmentConditionMatch(exeContext, fragment, type) {
  ...
  var conditionalType = (_typeFromAST.typeFromAST)(exeContext.schema, typeConditionNode);
  if (conditionalType === type) {
    return true;
  }
  if ((_definition.isAbstractType)(conditionalType)) {
...
function resolveFieldValueOrError(exeContext, fieldDef, fieldNodes, resolveFn, source, info) {
  try {
    ...
    var args = (_values.getArgumentValues)(fieldDef, fieldNodes[0], exeContext.variableValues);
...
function completeValueWithLocatedError(exeContext, returnType, fieldNodes, info, path, result) {
  try {
    ...
    if (promise) {
      return promise.then(undefined, function (error$$2) {
        return Promise.reject((_error.locatedError)(error$$2, fieldNodes, responsePathAsArray(path)));
      });
    }
    return completed;
  } catch (error$$2) {
    throw (_error.locatedError)(error$$2, fieldNodes, responsePathAsArray(path));
  }
}
...
function completeValue(exeContext, returnType, fieldNodes, info, path, result) {
  ...
  if ((_definition.isLeafType)(returnType)) {
    return completeLeafValue(returnType, result);
  }

  // If field type is an abstract type, Interface or Union, determine the
  // runtime Object type and complete for that type.
  if ((_definition.isAbstractType)(returnType)) {
...
function completeListValue(exeContext, returnType, fieldNodes, info, path, result) {
  !(_iterall.isCollection)(result) ? (_invariant2.default)(0, 'Expected Iterable, but did not find one for field ' + info.parentType.name + '.' + info.fieldName + '.') : void 0;
  ...
  (_iterall.forEach)(result, function (item, index) {
...
function completeLeafValue(returnType, result) {
  !returnType.serialize ? (_invariant2.default)(0, 'Missing serialize method on type') : void 0;
  var serializedResult = returnType.serialize(result);
  if ((_isNullish2.default)(serializedResult)) {
...
function graphqlImpl(schema, source, rootValue, contextValue, variableValues, operationName, fieldResolver) {
  return new Promise(function (resolve) {
    ...
    // Validate
    var validationErrors = (_validate.validate)(schema, document);
    if (validationErrors.length > 0) {
      return resolve({ errors: validationErrors });
    }

    // Execute
    resolve((_execute.execute)(schema, document, rootValue, contextValue, variableValues, operationName, fieldResolver));
  });
}
});
...
function mapAsyncIterator(iterable, callback, rejectCallback) {
  var iterator = (_iterall.getAsyncIterator)(iterable);
  var $return = void 0;
...
function subscribeImpl(schema$$2, document, rootValue, contextValue, variableValues, operationName, fieldResolver, subscribeFieldResolver) {
  ...
  var mapSourceToResponse = function mapSourceToResponse(payload) {
    return (_execute.execute)(schema$$2, document, payload, contextValue, variableValues, operationName, fieldResolver);
  };

  // Resolve the Source Stream, then map every source value to a
  // ExecutionResult value as described above.
  return sourcePromise.then(function (sourceStream) {
    return (_mapAsyncIterator2.default)(sourceStream, mapSourceToResponse, reportGraphQLError);
  }, reportGraphQLError);
}
...
function createSourceEventStream(schema$$2, document, rootValue, contextValue, variableValues, operationName, fieldResolver) {
  ...
  (_execute.assertValidExecutionArguments)(schema$$2, document, variableValues);

  return new Promise(function (resolve, reject) {
    // If a valid context cannot be created due to incorrect arguments,
    // this will throw an error.
    var exeContext = (_execute.buildExecutionContext)(schema$$2, document, rootValue, contextValue, variableValues, operationName, fieldResolver);

    var type = (_execute.getOperationRootType)(schema$$2, exeContext.operation);
    var fields = (_execute.collectFields)(exeContext, type, exeContext.operation.selectionSet, Object.create(null), Object.create(null));
    var responseNames = Object.keys(fields);
    var responseName = responseNames[0];
    var fieldNodes = fields[responseName];
    var fieldNode = fieldNodes[0];
    var fieldDef = (_execute.getFieldDef)(schema$$2, type, fieldNode.name.value);
    !fieldDef ? (_invariant2.default)(0, 'This subscription is not defined by the schema.') : void 0;

    // Call the `subscribe()` resolver or the default resolver to produce an
    // AsyncIterable yielding raw payloads.
    var resolveFn = fieldDef.subscribe || exeContext.fieldResolver;

    var path = (_execute.addPath)(undefined, responseName);

    var info = (_execute.buildResolveInfo)(exeContext, fieldDef, fieldNodes, type, path);

    // resolveFieldValueOrError implements the "ResolveFieldEventStream"
    // algorithm from GraphQL specification. It differs from
    // "ResolveFieldValue" due to providing a different `resolveFn`.
    Promise.resolve((_execute.resolveFieldValueOrError)(exeContext, fieldDef, fieldNodes, resolveFn, rootValue, info)).then(function (subscription) {
      // Reject with a located GraphQLError if subscription source fails
      // to resolve.
      if (subscription instanceof Error) {
        var error = (_locatedError.locatedError)(subscription, fieldNodes, (_execute.responsePathAsArray)(path));
        reject(error);
      }

      if (!(_iterall.isAsyncIterable)(subscription)) {
...
function buildClientSchema(introspection$$2) {
  ...
  var typeIntrospectionMap = (_keyMap2.default)(schemaIntrospection.types, function (type) {
    return type.name;
  });
  ...
  function getType(typeRef) {
    ...
    if (typeRef.kind === introspection.TypeKind.NON_NULL) {
      ...
      !!(nullableType instanceof definition.GraphQLNonNull) ? (_invariant2.default)(0, 'No nesting nonnull.') : void 0;
  ...
  function getInputType(typeRef) {
    ...
    !(_definition.isInputType)(type) ? (_invariant2.default)(0, 'Introspection must provide input type for arguments.') : void 0;
    return type;
  }

  function getOutputType(typeRef) {
    var type = getType(typeRef);
    !(_definition.isOutputType)(type) ? (_invariant2.default)(0, 'Introspection must provide output type for fields.') : void 0;
    return type;
  }

  function getObjectType(typeRef) {
    var type = getType(typeRef);
    !(type instanceof definition.GraphQLObjectType) ? (_invariant2.default)(0, 'Introspection must provide object type for possibleTypes.') : void 0;
    return type;
  }

  function getInterfaceType(typeRef) {
    var type = getType(typeRef);
    !(type instanceof definition.GraphQLInterfaceType) ? (_invariant2.default)(0, 'Introspection must provide interface type for interfaces.') : void 0;
    return type;
  }
  ...
  function buildEnumDef(enumIntrospection) {
    return new definition.GraphQLEnumType({
      ...
      values: (_keyValMap2.default)(enumIntrospection.enumValues, function (valueIntrospection) {
  ...
  function buildFieldDefMap(typeIntrospection) {
    return (_keyValMap2.default)(typeIntrospection.fields, function (fieldIntrospection) {
  ...
  function buildInputValueDefMap(inputValueIntrospections) {
    return (_keyValMap2.default)(inputValueIntrospections, function (inputValue) {
  ...
  function buildInputValue(inputValueIntrospection) {
    ...
    var defaultValue = inputValueIntrospection.defaultValue ? (_valueFromAST.valueFromAST)((_parser.parseValue)(inputValueIntrospection.defaultValue), type) : undefined;
    return {
...
function buildWrappedType(innerType, inputTypeNode) {
  ...
  if (inputTypeNode.kind === Kind.NON_NULL_TYPE) {
    ...
    !!(wrappedType instanceof definition.GraphQLNonNull) ? (_invariant2.default)(0, 'No nesting nonnull.') : void 0;
...
function buildASTSchema(ast) {
  ...
  function getObjectType(typeNode) {
    ...
    !(type instanceof definition.GraphQLObjectType) ? (_invariant2.default)(0, 'AST must provide object type.') : void 0;
    return type;
  }
  ...
  function produceInputType(typeNode) {
    return (_definition.assertInputType)(produceType(typeNode));
  }

  function produceOutputType(typeNode) {
    return (_definition.assertOutputType)(produceType(typeNode));
  }

  function produceObjectType(typeNode) {
    var type = produceType(typeNode);
    !(type instanceof definition.GraphQLObjectType) ? (_invariant2.default)(0, 'Expected Object type.') : void 0;
    return type;
  }

  function produceInterfaceType(typeNode) {
    var type = produceType(typeNode);
    !(type instanceof definition.GraphQLInterfaceType) ? (_invariant2.default)(0, 'Expected Interface type.') : void 0;
    return type;
  }
  ...
  function makeFieldDefMap(def) {
    return (_keyValMap2.default)(def.fields, function (field) {
      return field.name.value;
    }, function (field) {
      return {
  ...
  function makeInputValues(values) {
    return (_keyValMap2.default)(values, function (value) {
      ...
      return {
        ...
        defaultValue: (_valueFromAST.valueFromAST)(value.defaultValue, type),
        astNode: value
      };
    });
  }
  ...
  function makeEnumDef(def) {
    return new definition.GraphQLEnumType({
      name: def.name.value,
      description: getDescription(def),
      values: (_keyValMap2.default)(def.values, function (enumValue) {
...
function getDeprecationReason(node) {
  var deprecated = (_values.getDirectiveValues)(directives.GraphQLDeprecatedDirective, node);
...
function buildSchema(source) {
  return buildASTSchema((_parser.parse)(source));
}
...
function extendSchema(schema$$2, documentAST) {
  !(schema$$2 instanceof schema.GraphQLSchema) ? (_invariant2.default)(0, 'Must provide valid GraphQLSchema') : void 0;

  !(documentAST && documentAST.kind === Kind.DOCUMENT) ? (_invariant2.default)(0, 'Must provide valid Document AST') : void 0;
  ...
  function getMergedDirectives() {
    ...
    !existingDirectives ? (_invariant2.default)(0, 'schema must have default directives') : void 0;
  ...
  function getTypeFromDef(typeDef) {
    ...
    !type ? (_invariant2.default)(0, 'Missing type from schema') : void 0;
    return type;
  }
  ...
  function getObjectTypeFromAST(node) {
    ...
    !(type instanceof definition.GraphQLObjectType) ? (_invariant2.default)(0, 'Must be Object type.') : void 0;
    return type;
  }

  function getInterfaceTypeFromAST(node) {
    var type = getTypeFromAST(node);
    !(type instanceof definition.GraphQLInterfaceType) ? (_invariant2.default)(0, 'Must be Interface type.') : void 0;
    return type;
  }

  function getInputTypeFromAST(node) {
    return (_definition.assertInputType)(getTypeFromAST(node));
  }

  function getOutputTypeFromAST(node) {
    return (_definition.assertOutputType)(getTypeFromAST(node));
  }
  ...
  function extendFieldMap(type) {
    ...
    Object.keys(oldFieldMap).forEach(function (fieldName) {
      ...
      newFieldMap[fieldName] = {
        ...
        args: (_keyMap2.default)(field.args, function (arg) {
          return arg.name;
        }),
    ...
    if (extensions) {
      extensions.forEach(function (extension) {
        extension.definition.fields.forEach(function (field) {
          ...
          newFieldMap[fieldName] = {
            description: (_buildASTSchema.getDescription)(field),
            type: buildOutputFieldType(field.type),
            args: buildInputValues(field.arguments),
            deprecationReason: (_buildASTSchema.getDeprecationReason)(field),
            astNode: field
          };
        });
      });
  ...
  function buildObjectType(typeNode) {
    return new definition.GraphQLObjectType({
      ...
      description: (_buildASTSchema.getDescription)(typeNode),
      interfaces: function interfaces() {
  ...
  function buildEnumType(typeNode) {
    return new definition.GraphQLEnumType({
      name: typeNode.name.value,
      description: (_buildASTSchema.getDescription)(typeNode),
      values: (_keyValMap2.default)(typeNode.values, function (enumValue) {
        return enumValue.name.value;
      }, function (enumValue) {
        return {
          description: (_buildASTSchema.getDescription)(enumValue),
          deprecationReason: (_buildASTSchema.getDeprecationReason)(enumValue),
          astNode: enumValue
        };
      }),
      astNode: typeNode
  ...
  function getDirective(directiveNode) {
    return new directives.GraphQLDirective({
      name: directiveNode.name.value,
      description: (_buildASTSchema.getDescription)(directiveNode),
      locations: directiveNode.locations.map(function (node) {
        return node.value;
      }),
  ...
  function buildFieldMap(typeNode) {
    return (_keyValMap2.default)(typeNode.fields, function (field) {
      return field.name.value;
    }, function (field) {
      return {
        type: buildOutputFieldType(field.type),
  ...
  function buildInputValues(values) {
    return (_keyValMap2.default)(values, function (value) {
      ...
      return {
        type: type,
        description: (_buildASTSchema.getDescription)(value),
        defaultValue: (_valueFromAST.valueFromAST)(value.defaultValue, type),
        astNode: value
      };
    });
  ...
  function buildInputFieldType(typeNode) {
    ...
    if (typeNode.kind === Kind.NON_NULL_TYPE) {
      ...
      !!(nullableType instanceof definition.GraphQLNonNull) ? (_invariant2.default)(0, 'Must be nullable') : void 0;
      return new definition.GraphQLNonNull(nullableType);
    }
...
function printType(type) {
  ...
  !(type instanceof definition.GraphQLInputObjectType) ? (_invariant2.default)(0) : void 0;
...
function printInputValue(arg) {
  ...
  if (!(_isInvalid2.default)(arg.defaultValue)) {
    argDecl += ' = ' + (_printer.print)((_astFromValue.astFromValue)(arg.defaultValue, arg.type));
  }
  return argDecl;
}
...
function printDeprecated(fieldOrEnumVal) {
  ...
  if ((_isNullish2.default)(reason)) {
    return '';
  }
  if (reason === '' || reason === directives.DEFAULT_DEPRECATION_REASON) {
    return ' @deprecated';
  }
  return ' @deprecated(reason: ' + (_printer.print)((_astFromValue.astFromValue)(reason, scalars.GraphQLString)) + ')';
}
  ...
  Object.keys(oldTypeMap).forEach(function (typeName) {
    ...
    Object.keys(oldTypeFieldsDef).forEach(function (fieldName) {
      ...
      if (!(fieldName in newTypeFieldsDef)) {
        ...
        if (!isSafe) {
          var oldFieldTypeString = (_definition.isNamedType)(oldFieldType) ? oldFieldType.name : oldFieldType.toString();
          var newFieldTypeString = (_definition.isNamedType)(newFieldType) ? newFieldType.name : newFieldType.toString();
          breakingFieldChanges.push({
...
function isChangeSafeForObjectOrInterfaceField(oldType, newType) {
  if ((_definition.isNamedType)(oldType)) {
    return (
      // if they're both named types, see if their names are equivalent
      (_definition.isNamedType)(newType) && oldType.name === newType.name ||
...
function isChangeSafeForInputObjectFieldOrFieldArg(oldType, newType) {
  if ((_definition.isNamedType)(oldType)) {
    // if they're both named types, see if their names are equivalent
    return (_definition.isNamedType)(newType) && oldType.name === newType.name;
  } else if (oldType instanceof definition.GraphQLList) {
    // if they're both lists, make sure the underlying types are compatible
...
function findDeprecatedUsages(schema$$2, ast) {
  ...
  (_visitor.visit)(ast, (_visitor.visitWithTypeInfo)(typeInfo, {
    Field: function Field(node) {
    ...
    EnumValue: function EnumValue(node) {
      ...
      if (enumVal && enumVal.isDeprecated) {
        var type = (_definition.getNamedType)(typeInfo.getInputType());
        if (type) {
